...
var Pattern = function(subdir, filename, data) {
    this.filename = filename.substring(0, filename.indexOf('.'));
    this.subdir = subdir;
    this.name = (subdir.replace(/[\/\\]/g, '-') + '-' + this.filename).replace(/\\/g, '-');
    this.data = data || null;
    this.patterName = this.filename.substring(this.filename.indexOf('-') + 1); // ui display name
...
module.exports = function(opts, callback) {
    ...
    // }
    diveSync(baseDir, function (err, file) {
        console.log(file);
        // log errors
        if (err) {
            gutil.log(err);
        ...
        var abspath = file;
        var subdir = path.dirname(path.relative(baseDir, file));
        var filename = path.basename(file);
        var extRegex = new RegExp(/\.md$/i);
        ...
        var mypattern = new Pattern(subdir, filename);
